...
class JwtTokenProvider {
    ...
    private var validityInMillisecounds: Long = 3_600_000 //1h
    ...
    fun createAccessToken(username: String, roles: List<String?>): TokenVO {
        val now = Date()
        val validity = Date(now.time + validityInMillisecounds)
    ...
    private fun getAccessToken(username: String, roles: List<String?>, now: Date, validity: Date): String {
        val issuerUrl = ServletUriComponentsBuilder.fromCurrentContextPath().build().toString()
    ...
    private fun getRefreshToken(username: String, roles: List<String?>, now: Date): String? {
        val validityRefreshToken = Date(now.time + validityInMillisecounds * 3)
    ...
    fun validateExpireDateToken(token: String): Boolean {
